...
```{r bind-2}
new_data <- data_frame(numbers = c(24, 25, 26), more_letters = c("x", "y", "z"))
...
```{r bind-col}
new_data <- data_frame(direction = c("N", "S", "E", "W"))
...
```{r join-data}
df1 <- data_frame(sample = c(1, 2, 3), measure1 = c(4.2, 5.3, 6.1))
df2 <- data_frame(sample = c(1, 3, 4), measure2 = c(7.8, 6.4, 9.0))
