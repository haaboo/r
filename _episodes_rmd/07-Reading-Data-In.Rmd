...
To load this data into R, we will use the [`readr`](http://readr.tidyverse.org) package (which will 
...
> **How should we deal with examples that we don't intend to provide example files for 
> (such as this section)? For now I am just going to show what an example file would look like and 
> then refer to it with a descriptive name in the example code**
{: .discussion}
...
By default, `readr` uses the first row of data as column names for the data frame. If this is not
the case you can either specify them with a character vector, or have the column names generated 
automatically
...
> >
> > ```r
> > gapminder_tsv <- read_tsv("data/gapminder.tsv", col_types = cols(continent = col_factor(), year = col_date(format = "%Y")))
> >
> > all.equal(gapminder_csv, gapminder_tsv)
> > ```
> > ~~~~
...
Reading excel files in is very similar to the plain text files we have been using above.
